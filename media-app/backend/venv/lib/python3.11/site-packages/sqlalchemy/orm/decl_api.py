...
def _setup_declarative_base(cls: Type[Any]) -> None:
    ...
    if getattr(cls, "__init__.py", object.__init__) is object.__init__:
...
class DeclarativeBase(
    ...
    **__init__.py behavior**
    ...
    In a plain Python class, the base-most ``__init__.py()`` method in the class
    hierarchy is ``object.__init__.py()``, which accepts no arguments. However,
    when the :class:`_orm.DeclarativeBase` subclass is first declared, the
    class is given an ``__init__.py()`` method that links to the
    :paramref:`_orm.registry.constructor` constructor function, if no
    ``__init__.py()`` method is already present; this is the usual declarative
    ...
    **never accessed by a mapped class without being called explicitly via
    super()**, as mapped classes are themselves given an ``__init__.py()`` method
    directly which calls :paramref:`_orm.registry.constructor`, so in the
    default case works independently of what the base-most ``__init__.py()``
    method does.
    ...
    .. versionchanged:: 2.0.1  :class:`_orm.DeclarativeBase` has a default
       ...
       default, so that calls to ``super().__init__.py()`` can access this
       constructor. Previously, due to an implementation mistake, this default
       constructor was missing, and calling ``super().__init__.py()`` would invoke
       ``object.__init__.py()``.

    The :class:`_orm.DeclarativeBase` subclass may also declare an explicit
    ``__init__.py()`` method which will replace the use of the
    :paramref:`_orm.registry.constructor` function at this level::

        class Base(DeclarativeBase):
            def __init__.py(self, id=None):
                self.id = id

    Mapped classes still will not invoke this constructor implicitly; it
    remains only accessible by calling ``super().__init__.py()``::

        class MyClass(Base):
            def __init__.py(self, id=None, name=None):
                self.name = name
                super().__init__.py(id=id)
