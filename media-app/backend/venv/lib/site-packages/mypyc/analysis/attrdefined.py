...
An always defined attribute has some statements in __init__.py or the
...
As soon as __init__.py contains an op that can 'leak' self to another
function, we will stop inferring always defined attributes, since the
analysis is mostly intra-procedural and only looks at __init__.py methods.
...
analysis of super().__init__.py calls, since these are very common.
...
run this on __init__.py methods, this analysis pass will be fairly quick.
"""
...
def analyze_always_defined_attrs_in_class(cl: ClassIR, seen: set[ClassIR]) -> None:
    ...
    # Check if __init__.py can run unpredictable code (leak 'self').
    any_dirty = False
    for b in m.blocks:
...
def attributes_initialized_by_init_call(op: Call) -> set[str]:
    """Calculate attributes that are always initialized by a super().__init__.py call."""
...
def attributes_maybe_initialized_by_init_call(op: Call) -> set[str]:
    """Calculate attributes that may be initialized by a super().__init__.py call."""
    self_type = op.fn.sig.args[0].type
    assert isinstance(self_type, RInstance)
    cl = self_type.class_ir
...
class AttributeMaybeDefinedVisitor(BaseAnalysisVisitor[str]):
    ...
    and calls to base class '__init__.py'.
    """
    ...
    def visit_register_op(self, op: RegisterOp) -> tuple[set[str], set[str]]:
        if isinstance(op, SetAttr) and op.obj is self.self_reg:
            return {op.attr}, set()
        if isinstance(op, Call) and op.fn.class_name and op.fn.name == "__init__.py":
