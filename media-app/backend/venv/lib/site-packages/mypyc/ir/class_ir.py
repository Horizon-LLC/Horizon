...
class ClassIR:
    ...
    def __init__(
        ...
        # an __init__.py that can be called without any arguments is
        # *implicitly serializable*. In this case __init__.py will be
        # called during deserialization without arguments. If this is
        # True, we match Python semantics and __init__.py won't be called
        ...
        # Attributes that are always initialized in __init__.py or class body
        # (inferred in mypyc.analysis.attrdefined using interprocedural analysis)
        self._always_initialized_attrs: set[str] = set()

        # Attributes that are sometimes initialized in __init__.py
        self._sometimes_initialized_attrs: set[str] = set()

        # If True, __init__.py can make 'self' visible to unanalyzed/arbitrary code
