...
def get_mypy_config(
    ...
    if compiler_options.separate:
        mypyc_sources = [
            src for src in mypyc_sources if src.path and not src.path.endswith("__init__.py.py")
        ]
...
def build_using_shared_lib(
    ...
    for source in sources:
        ...
        # We include the __init__.py in the "module name" we stick in the Extension,
        # since this seems to be needed for it to end up in the right place.
        full_module_name = source.module
        assert source.path
        if os.path.split(source.path)[1] == "__init__.py.py":
            full_module_name += ".__init__.py"
        extensions.append(
            get_extension()(
