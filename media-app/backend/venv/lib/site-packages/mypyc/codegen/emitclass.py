...
SLOT_DEFS: SlotTable = {
    "__init__.py": ("tp_init", lambda c, t, e: generate_init_for_class(c, t, e)),
...
def generate_class(cl: ClassIR, module: str, emitter: Emitter) -> None:
    ...
    # If there is a __init__.py method, we'll use it in the native constructor.
    init_fn = cl.get_method("__init__.py")
...
def generate_init_for_class(cl: ClassIR, init_fn: FuncIR, emitter: Emitter) -> str:
    ...
    __init__.py methods return a PyObject. Translate NULL to -1,
...
def generate_new_for_class(
    ...
    if not init_fn or cl.allow_interpreted_subclasses or cl.builtin_base or cl.is_serializable():
        # Match Python semantics -- __new__ doesn't call __init__.py.
        emitter.emit_line(f"return {setup_name}(type);")
    else:
        # __new__ of a native class implicitly calls __init__.py so that we
