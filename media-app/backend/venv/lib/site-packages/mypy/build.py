...
class BuildManager:
    ...
    def all_imported_modules_in_file(self, file: MypyFile) -> list[tuple[int, str, int]]:
        ...
        def correct_rel_imp(imp: ImportFrom | ImportAll) -> str:
            ...
            if rel == 0:
                return imp.id
            if os.path.basename(file.path).startswith("__init__.py."):
                rel -= 1
            if rel != 0:
...
def get_cache_names(id: str, path: str, options: Options) -> tuple[str, str, str | None]:
    ...
    is_package = os.path.basename(path).startswith("__init__.py.py")
    if is_package:
        prefix = os.path.join(prefix, "__init__.py")
...
class State:
    ...
    def parse_file(self, *, temporary: bool = False) -> None:
        ...
        if self.tree is not None:
            # The file was already parsed (in __init__.py()).
            return
    ...
    def verify_dependencies(self, suppressed_only: bool = False) -> None:
        ...
        for dep in all_deps:
            ...
            try:
                ...
                # fine-grained mode when an __init__.py.py is deleted, if a module
...
def in_partial_package(id: str, manager: BuildManager) -> bool:
    ...
    This checks if there is any existing parent __init__.py.pyi stub that
...
def skipping_ancestor(manager: BuildManager, id: str, path: str, ancestor_for: State) -> None:
    ...
    # TODO: Read the path (the __init__.py.py file) and return
...
def load_graph(
    ...
    for bs in sources:
        ...
        if st.id in graph:
            ...
            manager.errors.report(
                ...
                "b) adding `__init__.py.py` somewhere, c) using `--explicit-package-bases` or "
                "adjusting MYPYPATH",
                severity="note",
            )
    ...
    for st in new:
        ...
        if not manager.use_fine_grained_cache():
            ...
            # This way we could avoid overhead of cloning options in `State.__init__.py()`
            ...
            # comment about this in `State.__init__.py()`.
            added = []
                    ...
                    if newst.path:
                        ...
                        if newst_path in seen_files:
                            ...
                            manager.errors.report(
                                -1,
                                0,
                                "Common resolutions include: a) adding `__init__.py.py` somewhere, "
