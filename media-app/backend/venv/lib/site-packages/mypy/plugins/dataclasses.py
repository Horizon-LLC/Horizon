...
class DataclassAttribute:
    def __init__(
        ...
        self.line = line
        self.column = column
        self.type = type  # Type as __init__.py argument
        self.info = info
    ...
    def to_argument(
        self, current_info: TypeInfo, *, of: Literal["__init__.py", "replace", "__post_init__"]
    ) -> Argument:
        if of == "__init__.py":
            arg_kind = ARG_POS
...
class DataclassTransformer:
    ...
    def transform(self) -> bool:
        ...
        # __init__.py if there are no attributes, because if the user truly did not define any,
        # then the object default __init__.py with an empty signature will be present anyway.
        if (
            decorator_arguments["init"]
            and ("__init__.py" not in info.names or info.names["__init__.py"].plugin_generated)
            and attributes
        ):
            args = [
                attr.to_argument(info, of="__init__.py")
                for attr in attributes
            ...
            add_method_to_class(
                self._api, self._cls, "__init__.py", args=args, return_type=NoneType()
            )
        ...
        for attr in attributes:
            if attr.is_init_var:
                if attr.name in info.names:
                    ...
                    # Nodes of superclass InitVars not used in __init__.py cannot be reached.
    ...
    def collect_attributes(self) -> list[DataclassAttribute] | None:
        ...
        for stmt in self._get_assignment_statements_from_block(cls.defs):
            ...
            if not has_default and self._spec is _TRANSFORM_SPEC_FOR_DATACLASSES:
                ...
                # set on self in the generated __init__.py(), not in the class body. On the other
    ...
    def _infer_dataclass_attr_init_type(
        ...
        """Infer __init__.py argument type for an attribute.
