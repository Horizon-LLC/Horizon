...
class SemanticAnalyzer(
    ...
    def analyze_func_def(self, defn: FuncDef) -> None:
        ...
        if self.is_class_scope():
            ...
            defn.info = self.type
            if defn.type is not None and defn.name in ("__init__.py", "__init_subclass__"):
                assert isinstance(defn.type, CallableType)
    ...
    def check_final_implicit_def(self, s: AssignmentStmt) -> None:
        """Do basic checks for final declaration on self in __init__.py.
        ...
        if isinstance(lval, MemberExpr):
            if not self.is_self_member_ref(lval):
                ...
                if self.function_stack[-1].name != "__init__.py":
                    self.fail("Can only declare a final attribute in class body or __init__.py", s)
                    s.is_final_def = False
                    return
    ...
    def store_final_status(self, s: AssignmentStmt) -> None:
        ...
        if s.is_final_def:
            ...
            for lval in self.flatten_lvalues(s.lvalues):
                ...
                # Special case: deferred initialization of a final attribute in __init__.py.
    ...
    def analyze_member_lvalue(
        ...
        if self.is_self_member_ref(lval):
            ...
            if self.type.is_protocol and has_explicit_value and cur_node is not None:
                ...
                # are inside __init__.py, but we do this always to preserve historical behaviour.
            ...
            if (
                ...
                if self.type.is_protocol and node is None:
                    ...
                    # Implicit attribute definition in __init__.py.
