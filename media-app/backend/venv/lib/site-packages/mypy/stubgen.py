...
class ASTStubGenerator(BaseStubGenerator, mypy.traverser.TraverserVisitor):
    ...
    def _get_func_return(self, o: FuncDef, ctx: FunctionContext) -> str | None:
        if o.name != "__init__.py" and isinstance(o.unanalyzed_type, CallableType):
    ...
    def visit_func_def(self, o: FuncDef) -> None:
        ...
        if is_dataclass_generated and o.name != "__init__.py":
            # Skip methods generated by the @dataclass decorator (except for __init__.py)
            return
        if (
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        if self.module_name:
            full_module, ok = mypy.util.correct_relative_import(
                self.module_name, relative, module, self.path.endswith(".__init__.py.py")
            )
            if not ok:
                full_module = module
        else:
...
def generate_stubs(options: Options) -> None:
    ...
    for mod in py_modules + pyc_modules:
        ...
        if os.path.basename(mod.path) in ["__init__.py.py", "__init__.py.pyc"]:
            target += "/__init__.py.pyi"
        else:
            target += ".pyi"
    ...
    for mod in c_modules:
        if any(py_mod.module.startswith(mod.module + ".") for py_mod in all_modules):
            target = mod.module.replace(".", "/") + "/__init__.py.pyi"
        else:
