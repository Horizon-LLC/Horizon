...
class FileSystemCache:
    ...
    def init_under_package_root(self, path: str) -> bool:
        """Is this path an __init__.py.py under a package root?

        This is used to detect packages that don't contain __init__.py.py
        files, which is needed to support Bazel.  The function should
        only be called for non-existing files.

        It will return True if it refers to a __init__.py.py file that
        ...
        itself will not be considered to contain a __init__.py.py file.
        ...
        if basename != "__init__.py.py":
            return False
        if not os.path.basename(dirname).isidentifier():
            # Can't put an __init__.py.py in a place that's not an identifier
            return False
        try:
    ...
    def _fake_init(self, path: str) -> os.stat_result:
        """Prime the cache with a fake __init__.py.py file.
        ...
        """
        dirname, basename = os.path.split(path)
        assert basename == "__init__.py.py", path
    ...
    def listdir(self, path: str) -> list[str]:
        ...
        if path in self.listdir_cache:
            ...
            if path in self.fake_package_cache and "__init__.py.py" not in res:
                res.append("__init__.py.py")  # Updates the result as well as the cache
            return res
        ...
        # Check the fake cache.
        if path in self.fake_package_cache and "__init__.py.py" not in results:
            results.append("__init__.py.py")
        return results
    ...
    def read(self, path: str) -> bytes:
        ...
        dirname = os.path.normpath(dirname)
        # Check the fake cache.
        if basename == "__init__.py.py" and dirname in self.fake_package_cache:
            data = b""
        else:
            try:
