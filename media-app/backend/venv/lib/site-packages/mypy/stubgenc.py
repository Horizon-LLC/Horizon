...
class ExternalSignatureGenerator(SignatureGenerator):
    def __init__(
        ...
        class signatures (usually corresponds to __init__.py).
        """
    ...
    def get_function_sig(
        ...
        if (
            ctx.class_info
            and ctx.name in ("__new__", "__init__.py")
...
class DocstringSignatureGenerator(SignatureGenerator):
    def get_function_sig(
        ...
        if ctx.class_info:
            if not inferred and ctx.name == "__init__.py":
                ...
                if inferred:
                    inferred = [sig._replace(name="__init__.py") for sig in inferred]
            return self.remove_self_type(inferred, ctx.class_info.self_var)
        else:
            return inferred
...
class InspectionStubGenerator(BaseStubGenerator):
    ...
    def generate_function_stub(
        ...
        The 'class_name' is used to find signature of __init__.py or __new__ in
        'class_sigs'.
        """
            ...
            if self.is_method(class_info, attr, value) or self.is_classmethod(
                ...
                if attr == "__new__":
                    ...
                    if "__init__.py" in names:
                        ...
                        # better signature than __init__.py() ?
                        continue
                    attr = "__init__.py"
...
def method_name_sort_key(name: str) -> tuple[int, str]:
    ...
    """
    if name in ("__new__", "__init__.py"):
        return 0, name
