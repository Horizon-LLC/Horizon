...
def type_object_type_from_function(
    ...
    # We first need to record all non-trivial (explicit) self types in __init__.py,
    ...
    # The __init__.py method might come from a generic superclass 'def_info'
    # with type variables that do not map identically to the type variables of
    # the class 'info' being constructed. For example:
    #
    #   class A(Generic[T]):
    #       def __init__.py(self, x: T) -> None: ...
    #   class B(A[List[T]]):
    #      ...
    #
    # We need to map B's __init__.py to the type (List[T]) -> None.
    signature = bind_self(
        signature,
    ...
    if isinstance(signature, CallableType):
        ...
        # Overloaded __init__.py/__new__.
...
def class_callable(
    ...
    """Create a type object type based on the signature of __init__.py."""
    ...
    if (
        ...
        # Only use the declared return type from __new__ or declared self in __init__.py
