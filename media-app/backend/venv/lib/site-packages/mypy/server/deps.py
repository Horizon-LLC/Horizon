...
class DependencyVisitor(TraverserVisitor):
        ...
        with self.scope.function_scope(o):
            ...
            if o.info:
                for base in non_trivial_bases(o.info):
                    # Base class __init__.py/__new__ doesn't generate a logical
                    # dependency since the override can be incompatible.
                    if not self.use_logical_deps() or o.name not in ("__init__.py", "__new__"):
    ...
    def process_type_info(self, info: TypeInfo) -> None:
        ...
        for base_info in non_trivial_bases(info):
            for name, node in base_info.names.items():
                if self.use_logical_deps():
                    ...
                    if name not in info.names:
                        continue
                    # __init__.py and __new__ can be overridden with different signatures, so no
                    # logical dependency.
                    if name in ("__init__.py", "__new__"):
                        continue
                self.add_dependency(
                    make_trigger(base_info.fullname + "." + name),
                    target=make_trigger(info.fullname + "." + name),
            ...
            if not self.use_logical_deps():
                ...
                # they aren't logical dependencies since __init__.py and __new__ can be
                # overridden with a different signature.
                self.add_dependency(
                    make_trigger(base_info.fullname + ".__init__.py"),
                    target=make_trigger(info.fullname + ".__init__.py"),
                )
                self.add_dependency(
    ...
    def visit_assignment_stmt(self, o: AssignmentStmt) -> None:
        ...
        if isinstance(rvalue, CallExpr) and isinstance(rvalue.analyzed, TypeVarExpr):
            ...
            if isinstance(typ, FunctionLike) and typ.is_type_obj():
                ...
                self.add_dependency(make_trigger(class_name + ".__init__.py"))
        ...
        if self.use_logical_deps() and o.unanalyzed_type is None:
            ...
            if (
                ...
                if isinstance(rvalue.callee.node, TypeInfo):
                    # use actual __init__.py as a dependency source
                    init = rvalue.callee.node.get("__init__.py")
    ...
    def visit_member_expr(self, e: MemberExpr) -> None:
        if isinstance(e.expr, RefExpr) and isinstance(e.expr.node, TypeInfo):
            # Special case class attribute so that we don't depend on "__init__.py".
    ...
    def process_isinstance_call(self, e: CallExpr) -> None:
        ...
        if len(e.args) == 2:
            ...
            if (
                ...
                # Special case to avoid redundant dependencies from "__init__.py".
    ...
    def visit_type_type(self, typ: TypeType) -> list[str]:
        ...
        if not self.use_logical_deps:
            ...
            for trigger in old_triggers:
                triggers.append(trigger.rstrip(">") + ".__init__.py>")
