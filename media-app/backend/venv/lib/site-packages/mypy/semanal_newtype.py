...
class NewTypeAnalyzer:
    ...
    def build_newtype_typeinfo(
        ...
        init_func = FuncDef("__init__.py", args, Block([]), typ=signature)
        init_func.info = info
        init_func._fullname = info.fullname + ".__init__.py"
        if not existing_info:
            updated = True
        else:
            previous_sym = info.names["__init__.py"].node
            assert isinstance(previous_sym, FuncDef)
            updated = old_type != previous_sym.arguments[1].variable.type
        info.names["__init__.py"] = SymbolTableNode(MDEF, init_func)
