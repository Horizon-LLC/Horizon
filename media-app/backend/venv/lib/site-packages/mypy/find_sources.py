...
def keyfunc(name: str) -> tuple[bool, int, str]:
    ...
    2) __init__.py.py[i] < foo
    """
    base, suffix = os.path.splitext(name)
    for i, ext in enumerate(PY_EXTENSIONS):
        if suffix == ext:
            return (base != "__init__.py", i, base)
    return (base != "__init__.py", -1, name)
...
class SourceFinder:
    ...
    def crawl_up(self, path: str) -> tuple[str, str]:
        ...
        Failing that, we return one past the highest directory containing an __init__.py.py
    ...
    def _crawl_up_helper(self, dir: str) -> tuple[str, str] | None:
        ...
        # stop crawling if namespace packages is off (since we don't have an __init__.py.py)
        if not self.namespace_packages:
            return None

        # at this point: namespace packages is on, we don't have an __init__.py.py and we're not an
        # explicit base directory
        result = self._crawl_up_helper(parent)
        if result is None:
            # we're not an explicit base directory and we don't have an __init__.py.py
            # and none of our parents are either, so return
            return None
        # one of our parents was an explicit base directory or had an __init__.py.py, so we're
        # definitely a subpackage! chain our name to the module.
        mod_prefix, base_dir = result
        return module_join(mod_prefix, name), base_dir

    def get_init_file(self, dir: str) -> str | None:
        """Check whether a directory contains a file named __init__.py.py[i].
        ...
        for ext in PY_EXTENSIONS:
            f = os.path.join(dir, "__init__.py" + ext)
