...
class FindModuleCache:
    ...
    def find_module_via_source_set(self, id: str) -> ModuleSearchResult | None:
        ...
        if p and self.fscache.isfile(p):
            # We need to make sure we still have __init__.py.py all the way up
            ...
            for _ in range(id.count(".")):
                if not any(
                    self.fscache.isfile(os.path.join(d, "__init__.py" + x)) for x in PYTHON_EXTENSIONS
                ):
                    return None
                d = os.path.dirname(d)
            return p
        ...
        if idx != -1:
            ...
            if not any(parent.endswith("__init__.py" + x) for x in PYTHON_EXTENSIONS) and (
                ...
                # hence the filtering out of __init__.py files, and checking for the presence
    ...
    def _find_module(self, id: str, use_typeshed: bool) -> ModuleSearchResult:
                ...
                if fscache.isdir(path):
                    if fscache.isfile(stub_typed_file):
                        ...
                        if fscache.read(stub_typed_file).decode().strip() == "partial":
                            ...
                            # the partial stub package may not have a __init__.py.pyi
        ...
        # Now just look for 'baz.pyi', 'baz/__init__.py.py', etc., inside those directories.
        seplast = os.sep + components[-1]  # so e.g. '/baz'
        sepinit = os.sep + "__init__.py"
        ...
        for base_dir, verify in candidate_base_dirs:
            ...
            # Prefer package over module, i.e. baz/__init__.py.py* over baz.py*.
        ...
        #
        # - xxx/foo/bar/baz.py
        # - yyy/foo/bar/baz/__init__.py.py
        # - zzz/foo/bar/baz.pyi
        #
        # If any of the foo directories has __init__.py.py[i], it wins.
        # Else, we look for foo/bar/__init__.py.py[i], etc.  If there are
        # none, the first hit wins.  Note that this does not take into
        # account whether the lowest-level module is a file (baz.py),
        # a package (baz/__init__.py.py), or a stub file (baz.pyi) -- for
        # these the first one encountered along the search path wins.
        #
        # The helper function highest_init_level() returns an int that
        # indicates the highest level at which a __init__.py.py[i] file
        # is found; if no __init__.py was found it returns 0, if we find
        # only foo/bar/__init__.py.py it returns 1, and if we have
        # foo/__init__.py.py it returns 2 (regardless of what's in
        ...
        for name in names:
            ...
            if self.fscache.isdir(subpath):
                ...
                if (self.options and self.options.namespace_packages) or (
                    self.fscache.isfile(os.path.join(subpath, "__init__.py.py"))
                    or self.fscache.isfile(os.path.join(subpath, "__init__.py.pyi"))
                ):
                    seen.add(name)
...
def is_init_file(path: str) -> bool:
    return os.path.basename(path) in ("__init__.py.py", "__init__.py.pyi")


def verify_module(fscache: FileSystemCache, id: str, path: str, prefix: str) -> bool:
    """Check that all packages containing id have a __init__.py file."""
    if is_init_file(path):
        path = os.path.dirname(path)
    for i in range(id.count(".")):
        path = os.path.dirname(path)
        if not any(
            fscache.isfile_case(os.path.join(path, f"__init__.py{extension}"), prefix)
            for extension in PYTHON_EXTENSIONS
        ):
            return False
    return True
...
def highest_init_level(fscache: FileSystemCache, id: str, path: str, prefix: str) -> int:
    """Compute the highest level where an __init__.py file is found."""
    if is_init_file(path):
        path = os.path.dirname(path)
    level = 0
    for i in range(id.count(".")):
